...
int main() {
    ...
    answer: "30",
    explanation:
      "p는 b를 가리키므로 p->val은 20이고, p->next는 a의 주소(&a)이므로 p->next->val은 10입니다. 따라서 20 + 10 = 30이 출력됩니다.",
    difficulty: "MEDIUM",
  ...
  },
  {
    ...
    type: "SQL",
    question:
      "다음 SQL 쿼리에서 평균 급여보다 많은 급여를 받는 사원의 이름을 조회하고자 한다. 빈칸에 들어갈 올바른 SQL 절을 완성하시오.\n\nSELECT 이름 \nFROM 급여 \nWHERE 금액 > (   빈칸   );",
    answer: [
      "SELECT AVG(금액) FROM 급여",
      "SELECT AVG(금액)FROM 급여",
      "select avg(금액) from 급여",
    ],
    explanation:
      "서브쿼리를 사용하여 전체 사원의 평균 급여(SELECT AVG(금액) FROM 급여)를 계산한 후, 그 값보다 큰 금액을 받는 사원을 조회합니다.",
    difficulty: "MEDIUM",
  ...
  },
  {
    ...
    type: "SHORT_ANSWER",
    question:
      "미국 국립표준기술연구소(NIST)에서 제정한 해시 암호화 표준으로, 256비트의 고정 길이 다이제스트를 출력하며 임의의 길이의 데이터를 복호화할 수 없는 일방향 해시 알고리즘의 명칭을 영문으로 쓰시오.",
    answer: ["SHA-256", "SHA256", "sha-256", "sha256"],
    explanation:
      "SHA-256은 SHA-2 계열의 해시 함수로서 충돌 저항성과 일방향성을 만족하며, 블록체인 및 보안 서명에 널리 사용되는 표준 해시 알고리즘입니다.",
    difficulty: "EASY",
...
public class Main {
    ...
    answer: "10,20",
    explanation:
      '필드(멤버 변수)는 참조 변수의 타입(A)을 따르므로 obj.x는 10이고, 오버라이딩된 메서드는 실제 객체(B)의 가상 메서드 테이블에 의해 B의 print()가 호출되어 20이 출력됩니다. 따라서 "10,20"입니다.',
    difficulty: "HARD",
...
print(total)`,
    ...
    answer: "15",
    explanation:
      'keys[0]은 "A"이므로 data["A"]는 10입니다. data.get("D", 5)는 딕셔너리에 "D" 키가 없으므로 기본값 5를 반환합니다. 따라서 10 + 5 = 15가 출력됩니다.',
    difficulty: "EASY",
  ...
  },
  {
    ...
    type: "SHORT_ANSWER",
    question:
      "소프트웨어 화이트박스 테스트 검증 기준(Coverage) 중 소스 코드 내부의 모든 가능한 분기(조건문의 참과 거짓)를 최소한 한 번씩 모두 실행하도록 테스트 케이스를 설계하는 검증 기준의 명칭을 쓰시오.",
    answer: [
      "분기 커버리지",
      "분기 검증",
      "결정 커버리지",
    ...
    explanation:
      "구문 커버리지(Statement Coverage)는 모든 문장을 최소 1회 실행하는 것이며, 분기 커버리지(Branch Coverage, 결정 커버리지)는 모든 조건문의 참(True)과 거짓(False) 갈래를 최소 1회 이상 통과하도록 하는 테스트 기준입니다.",
    difficulty: "MEDIUM",
    keywords: [
      "화이트박스",
      "분기 커버리지",
  ...
  },
  {
    ...
    type: "SHORT_ANSWER",
    question:
      "자율 시스템(AS, Autonomous System) 간에 경로를 교환하기 위해 BGP 프로토콜에서 사용하는 전송 계층 프로토콜 명칭과 기본 포트 번호(Port)를 순서대로 쓰시오 (예: UDP, 53).",
    answer: ["TCP, 179", "TCP,179", "tcp, 179", "tcp,179"],
    explanation:
      "BGP(Border Gateway Protocol)는 신뢰성 있는 대규모 라우팅 정보 교환을 위해 전송 계층으로 TCP 179번 포트를 사용합니다.",
    difficulty: "HARD",
  ...
  },
  {
    ...
    type: "SHORT_ANSWER",
    question:
      "데이터베이스 장애 발생 시 트랜잭션의 연산을 취소하고 변경 내용을 데이터베이스의 원래 상태로 되돌리는 작업을 일컫는 명칭을 영문 또는 한글로 쓰시오.",
    answer: ["UNDO", "undo", "취소"],
    explanation:
      "회복 기법에서 트랜잭션이 실패하여 이전 상태로 되돌리는 연산은 UNDO(취소)이며, 장애 이전의 성공한 트랜잭션 변경 내용을 재실행하여 일관성을 맞추는 연산은 REDO(재실행)입니다.",
    difficulty: "EASY",
...
export class QuestionSyncService {
  ...
  static async getCachedServerQuestions(): Promise<Question[]> {
    const cached = await LocalStorage.getItem<Question[]>(
